function aircraftPB() {
  ...
  //斜め着陸爆破判定
  {
    ...
    let rad2 = Math.atan(localXAxis.y/Math.sqrt(localXAxis.x**2+localXAxis.z**2));
    console.log(rad2);
    if (aircraft.y < 70 && Math.abs(rad2) > Math.PI/6) {
      aircraft.exploded = true;
    }
  }
  ...
  //エレベーター
  if (!aircraft.landingFlag) {
    let elevLift = aircraft.speed.z * aircraft.elevator / 7500;
    if (aircraft.speed.z < 3) {elevLift = 0};
    aircraft.rotationSpeed.x += (-(aircraft.speed.y/10000)-(elevLift) - aircraft.rotationSpeed.x)/20;
  }
  ...
  //ラダー
  let rudderLift = aircraft.speed.z * aircraft.rudder / 10000;
  ...
  //エルロン
  if (!aircraft.landingFlag) {
    let aileronLift = aircraft.speed.z * aircraft.aileron / 3000;
